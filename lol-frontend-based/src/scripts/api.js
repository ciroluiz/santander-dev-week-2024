...
async function loadChampions() {
  ...
  //2.guardar os dados recebidos
  state.values.champions = data;
}

async function renderChampions() {
  //3.renderizar imagens na tela
...
async function onChangeChampionSelected(id, imageUrl) {
  //1.trocar a imagem de fundo do círculo
...
async function getRandomQuote() {
  const quotes = [
    "Manda ver meu nobre 😎",
    "Pode vir quente que eu tô frevendo 🔥",
    "Aguardo sua pergunta ❓",
    "Espero ansiosamente pela sua pergunta ❔",
    "Estou começando a ficar com tédio 😑",
    "Tenho vidas a salvar, depressa com isso ‼",
    "Não vai ficar aí o dia todo, ou vai? 🙄",
